...
// ============================================================
// PHP BACKEND ENDPOINTS NEEDED:
//
// 1. POST /api/forgot-password.php
//    Request:  { "email": "user@mymail.mapua.edu.ph", "userType": "student" | "admin" }
//    Response: { "success": true, "message": "Verification code sent" }
//    Logic:    Validate email exists in DB (check specific table based on userType),
//              generate 6-digit OTP, store OTP + expiry (e.g. 10 min) in password_resets table,
//              send OTP via email (PHPMailer / SMTP)
//
// 2. POST /api/verify-code.php
//    Request:  { "email": "...", "code": "123456", "userType": "student" | "admin" }
//    Response: { "success": true, "reset_token": "abc123..." }
//    Logic:    Validate OTP matches DB record, check not expired,
//              generate a one-time reset_token, store in DB
//
// 3. POST /api/reset-password.php
//    Request:  { "email": "...", "reset_token": "abc123...", "new_password": "...", "userType": "student" | "admin" }
//    Response: { "success": true, "message": "Password updated" }
//    Logic:    Validate reset_token, hash new password (password_hash),
//              update correct users table based on userType, delete reset token from DB
//
// 4. POST /api/resend-code.php
//    Request:  { "email": "...", "userType": "student" | "admin" }
//    Response: { "success": true, "message": "New code sent" }
//    Logic:    Invalidate old OTP, generate new 6-digit OTP,
//              store with new expiry, send via email
// ============================================================
export function ForgotPasswordFlow({
  ...
  // --- API Call Helpers (structured for PHP backend) ---
  // helper that fetches JSON and provides readable debug info when parsing fails
  const fetchJson = async (url: string, options: RequestInit) => {
    ...
    try {
      ...
      console.error('Invalid JSON from', url, 'response text:', text);
      throw new Error('Invalid server response');
    }
    if (!response.ok) {
      // if server provided message in JSON, use it
    ...
    : Promise<{
      ...
      return await fetchJson(`${API_BASE}/api/forgot-password.php`, {
        ...
        body: JSON.stringify({
          email: emailAddress,
          userType,
          debug: process.env.NODE_ENV === 'development' // handy when running locally
        })
      });
    },
    [userType]
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        // if backend returned otp for debugging, log it (or show a toast)
        if (result.otp) {
          console.debug('DEBUG OTP from server:', result.otp);
        }
        setStep('otp');
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      if (result.success) {
        if (result.otp) {
          console.debug('DEBUG OTP from server (resend):', result.otp);
        }
        setOtp(['', '', '', '', '', '']);
        setResendCooldown(60);
      ...
      <AnimatePresence mode="wait">
        ...
        <motion.div
          ...
          className="bg-red-50 text-red-600 p-3 rounded-md text-sm flex items-start">
            ...
            <AlertCircleIcon className="w-5 h-5 mr-2 shrink-0 mt-0.5" />
            <span>{error}</span>
          </motion.div>
        }
      </AnimatePresence>
