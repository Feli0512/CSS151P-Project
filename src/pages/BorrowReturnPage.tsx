...
interface BorrowReturnPageProps {
  ...
  user: User;
  onBorrowSubmit: (transaction: Omit<Transaction, 'id' | 'status'>) => Promise<void>;
  onReturnSubmit: (transactionId: string) => Promise<void>;
}
export function BorrowReturnPage({
  ...
  const handleBorrowSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Frontend validation
      if (!borrowForm.equipmentId || !borrowForm.quantity || !borrowForm.purpose || !borrowForm.dateNeeded || !borrowForm.expectedReturn) {
      ...
      if (new Date(borrowForm.expectedReturn) < new Date(borrowForm.dateNeeded)) {
        setError('Expected return date cannot be before the borrow date');
        return;
      }

      await onBorrowSubmit({
      ...
      setError(error instanceof Error ? error.message : 'Failed to submit borrow request');
    } finally {
      setIsLoading(false);
    }
  };
  const handleReturnSubmit = async (transactionId: string) => {
    ...
    setError(null);
    try {
      await onReturnSubmit(transactionId);
      setSuccessMessage('Equipment returned successfully!');
      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 3000);
    } catch (error) {
      setError(error instanceof Error ? error.message : 'Failed to process return');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <AnimatePresence>
        ...
        <motion.div
          ...
          className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center text-red-800">
            ...
            <AlertCircleIcon className="w-5 h-5 mr-3 text-red-500" />
            {error}
          </motion.div>
        }
      </AnimatePresence>
                ...
                <form onSubmit={handleBorrowSubmit} className="space-y-6">
                  ...
                  <div className="pt-4 flex justify-end">
                    <Button type="submit" className="w-full md:w-auto px-8" disabled={isLoading}>
                      {isLoading ? 'Submitting...' : 'Submit Request'} <ArrowRightIcon className="w-4 h-4 ml-2" />
                    </Button>
                  </div>
                </form>
              </CardContent>
          ...
          <Card key={item.id} className="overflow-hidden">
                  <div className="flex flex-col md:flex-row">
                    ...
                    disabled={isLoading}>
                          ...
                          {isLoading ? 'Processing...' : 'Return Item'}
                        </Button>
                      </div>
                    </div>
                  </div>
